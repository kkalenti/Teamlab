...
for i = 1:realSize(2)
    ...
    real(i).ch3.coordinate = plot(Real(i).t(:),Real(i).z(:),'-o','Color',cmap(i+5,:),...
        'Visible','off','DisplayName', 'Эталон z');
    real(i).ch3.speed = plot(Real(i).t(:),Real(i).Vz(:),'-o','Color',cmap(i+5,:),...
        'Visible','off','DisplayName', 'Эталон V(z)');
    real(i).ch3.acceleration = plot(Real(i).t(:),Real(i).az(:),'-o','Color',cmap(i+5,:),...
