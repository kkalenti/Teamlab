...
//Первоначальное создание структуры
int createStruct(CReferenceState* s, int target) {
	...
	RealArray = mxCreateStructMatrix(1, target, 10, fieldNames);//???, Цели, измерения(x,y,z...)
...
int createStruct(CResultOfScan* s) {
	...
	POIArray = mxCreateStructMatrix(1, 1, 5, fieldNames);//???, Цели, измерения(x,y,z...)
...
int createStruct(CVoi2* s) {
	...
	VOIArray = mxCreateStructMatrix(1, 1, 10, fieldNames);//???, Цели, измерения(x,y,z...)
...
//Расширение field (новые данные по известной цели)
int extendArray(CReferenceState* s, mxArray *origin, int target) {
	...
	mx = mxCreateDoubleMatrix(1, n + 1, mxREAL);//расширяю
...
//Расширение структуры (новая цель)
int extendStruct(CReferenceState* s, mxArray *origin, int oldN, int target) {
	...
	extendedStruct = mxCreateStructMatrix(1, target, 10, fieldNames);//расширяем
	...
	//Заносим новые данные в только что расширенную структуру
	mx = mxCreateDoubleMatrix(1, 1, mxREAL);
	memcpy((void *)(mxGetPr(mx)), &s->Coordinate.x, sizeof(&s->Coordinate.x));
...
int extendStruct(CVoi2* s, mxArray *origin, int oldN) {
	...
	extendedStruct = mxCreateStructMatrix(1, oldN + 1, 10, fieldNames);//расширяем
